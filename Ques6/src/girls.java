...
public class girls {
     ...
     * @param n No. of boys
     * @param boy_index To store the index of previous boyfriend
     * @return The number of the boy who is suitable
     */
    public int matchMaker(boys b[],int n,int boy_index) {
        int max_attribute = 0 , i,k=boy_index;
        
        // if girl prefers most attractive available boy
        if(criteria == 1) {
            i=0;
            while(i<n){
                if(b[i].budget>=maintainance_cost && b[i].attractive>=max_attribute && b[i].status==1 && b[i].min_attraction<=attractive && i!=k)
                {
            ...
            i=0;
            while(i<n){
                if(b[i].budget>=maintainance_cost && b[i].budget>=max_attribute && b[i].status==1 && b[i].min_attraction<=attractive && i!=k)
                {
            ...
            i=0;
            while(i<n){
                if(b[i].budget>=maintainance_cost && b[i].intellect>=max_attribute && b[i].status==1 && b[i].min_attraction<=attractive && i!=k)
                {
