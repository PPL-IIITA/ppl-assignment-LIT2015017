...
public class input {
    ...
    public input(girls g[],boys b[],utility_gifts u[],luxury_gifts l[], essential_gift e[],int commit[], couple c[]) throws IOException{
        ...
        for(int i=0;i<gs;i++) {
            int x=g[i].matchMaker(b, bs,commit[i]);
            commit[i]=x;
    ...
    public void pair(girls g[],boys b[],int commit[],couple c[],int breakup[],int k,int lo){
       
        for(int i=lo-1,j=0;i>=lo-k;i--,j++) {
                if(i>=0)
                c[i].break_couple(breakup,j,g,commit,b);
                
        }
        //Assigning new boyfriend to the girls who broke-up
        for(int i=0;i<k;i++) {
            int y=g[breakup[i]].matchMaker(b, bs, commit[breakup[i]]);
            commit[breakup[i]]=y;
        }
        /*for(int i=0;i<gs;i++) {
            if(g[i].status==1) {
                int y=g[i].matchMaker(b, bs, commit[i]);
                commit[i]=y;
            }
        }*/
    ...
    public void gift(girls g[],boys b[],luxury_gifts l[],essential_gift e[])  throws IOException {
        File file = new File("coupledata.txt");
        FileWriter qwe= new FileWriter(file,true);
        for(int i=0;i< bs;i++) {
